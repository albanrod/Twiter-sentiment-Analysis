...
library(plotrix)
#install.packages("RJSONIO", repos = "https://cran.r-project.org/package=plotrix", type="source")
...
if (!require(x,character.only=TRUE))
{
  ...
  install.packages(pkgs=x,repos="https://cran.r-project.org/package=RJSONIO")
...
shinyServer(function(input, output, session) {
  consumer_key <- "3EH9Rd6AYu4gbb4g7dfoez1TD" # Enter your consumer key
  consumer_secret <- "fhvY9emcUstgW8Gn82dIgyxfWOvFRaQaFCWM4qg6tGlwMlLLqL" # Enter your consumer secret
  access_token <- "720157843589083137-y2tBVLxhJF38vlNil0QuF8Psr8TeSOq" # Enter your access token
  access_secret <- "ViX4proVUyso8RhmFEiQFN0Eowu4BDMWH8QtWhJ0onY3g" # Enter your access secret
  ...
  pos.words = scan('word.txt', what='character', comment.char=';') #Provide your path to "positive_words.txt" file
  neg.words = scan('word.txt', what='character', comment.char=';') #Provide your path to "negative_words.txt" file
  ...
  text_word<-reactive({text_word<-wordclouds( tweets()[0:80] )})
